...
function checkCrossOrigin({
  ...
  const fwdProtoHasMoreValuesThanFwdPorts = (forwardedProto || "").split(",").length > (forwardedPort || "").split(",").length;
  ...
  const finalURL = convertHostHeaderValueToURL(forwardedHost || host || void 0, protocol);
...
function convertHostHeaderValueToURL(host, protocol = "https") {
  return new URL(`${protocol}://${host}`);
}
var PROTOCOL_TO_PORT_MAPPING = {
  http: "80",
  https: "443"
};
...
function getFirstValueFromHeaderValue(value) {
  return value?.split(",")[0]?.trim() || "";
}
function getProtocolVerb(protocol) {
...
var potentialRequestAfterSignInOrOutFromClerkHostedUiInDev = (options) => {
  ...
  const crossOriginReferrer = referrer && checkCrossOrigin({ originURL: new URL(referrer), host, forwardedHost, forwardedPort, forwardedProto });
  const key = secretKey || apiKey || "";
