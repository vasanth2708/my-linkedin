...
const handleMultiDomainAndProxy = (req, opts) => {
  ...
  const relativeOrAbsoluteProxyUrl = handleValueOrFn(opts?.proxyUrl, requestURL, import_clerkClient.PROXY_URL);
  let proxyUrl;
  ...
  const isSatellite = handleValueOrFn(opts.isSatellite, new URL(req.url), import_clerkClient.IS_SATELLITE);
  const domain = handleValueOrFn(opts.domain, new URL(req.url), import_clerkClient.DOMAIN);
  const signInUrl = opts?.signInUrl || import_clerkClient.SIGN_IN_URL;
  if (isSatellite && !proxyUrl && !domain) {
    throw new Error(import_errors.missingDomainAndProxy);
  }
  if (isSatellite && !isHttpOrHttps(signInUrl) && isDevelopmentFromApiKey(import_clerkClient.SECRET_KEY || import_clerkClient.API_KEY)) {
